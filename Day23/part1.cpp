...
int max_path_DAG(const std::vector<std::vector<int>> costs)
{
    std::vector<int> distance(costs.size(), INT32_MAX);
    distance[0] = 0;
    ...
    std::priority_queue<Node, std::vector<Node>, Comparator> to_explore;
    to_explore.push(Node(0, 0));
    while (!to_explore.empty())
    {
        Node current = to_explore.top();
        to_explore.pop();
        int node_idx = std::get<0>(current), node_dist = std::get<1>(current);

        auto row = costs[node_idx];
        for (int i = 0; i < costs.size(); i++)
        {
            ...
            int new_dist = node_dist - row[i];
            if (new_dist < distance[i])
            {
                distance[i] = new_dist;
                to_explore.push(Node(i, new_dist));
            }
        }
    }
...
int solution()
{
    ...
    return max_path_DAG(cost_matrix);
}
