...
long solution()
{
    ...
    for (int i = 0; i < galaxies.size(); i++)
    {
        ...
        for (int j = i+1; j < galaxies.size(); j++)
        {
            ...
            long normal_dist = std::abs(j_row-i_row) + std::abs(j_col - i_col);
            long expansion = std::abs(cum_empty_rows[j_row]-cum_empty_rows[i_row]) + std::abs(cum_empty_cols[j_col] - cum_empty_cols[i_col]);
            total_distance += normal_dist + 999999*expansion;
        }
    }
